...
async function mostrarAudioLetraAcordes(audio_file, tono_audio, txt_file) {
  ...
  fileExist(audio_file).then(exist => {
    if (exist) {
      ...
      document.getElementById("bloque_audio").innerHTML += '<p style="color: grey;" id='audio_text'>Versión de audio por '+x(tono_audio)+'</p>'
    } else {
      document.getElementById("bloque_audio").innerHTML = '<br>'
      document.getElementById("bloque_audio").innerHTML += '<audio controls src="'+audio_file+'"></audio>'
      document.getElementById("bloque_audio").innerHTML += '<p style="color: grey;" id='audio_text'>Aun no hemos incluido un audio para esta canción</p>'
    }
});
